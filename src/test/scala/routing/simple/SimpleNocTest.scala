  ...
  "SimpleNoC" should {
    ...
    "route packets correctly" in {
      ...
      model.simulate(runDir) { dut =>
        ...
        val producers = Seq.tabulate(4)(i => new DecoupledProducer(dut.ports(i).ingress, dut.clock))
        val consumers = Seq.tabulate(4)(i => new DecoupledConsumer(dut.ports(i).egress, dut.clock))
        ...
        Task {
          producers(0).send(Seq(packetTo_1_0, packetTo_0_1, packetTo_1_1))
        }
